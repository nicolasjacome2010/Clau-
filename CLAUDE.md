...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth/Home/Mis Decisiones (Pantallas 1, 2, 3, 4, 10 of §2) are implemented** (see `app/`); Pantallas 5-9 and 11-15 (Clarificación, Simulación en vivo, Resultados, Memoria, Suscripción, Ajustes) are still design-only.

Only **Core API modules 1-6 ("identity", "goals", "decisions", "simulations", "memory", "billing")**, **Reality Engine Agents 0-12 (all 13)**, and the **Flutter client's foundation + three feature verticals** (`app/`: design system, routing, networking scaffold, Splash → Onboarding → Auth → Home → Mis Decisiones) have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
app/             Flutter client — SEPARATE Dart/Flutter project, not part of backend/ or reality_engine/
  ...
  lib/
    ...
    features/home/                         # Pantalla 4, REAL: nav shell (bottom nav mobile /
                                           # NavigationRail tablet+) has 2 of its 4 destinations as
                                           # placeholders (Memoria/Perfil); Home + Mis Decisiones are real
