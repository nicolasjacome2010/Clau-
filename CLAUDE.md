...
Only **Phase 6, modules 1-3 ("identity", "goals", "decisions")** of the backend have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
There is no Flutter client, Reality Engine service, Billing service, `simulations`, or `memory` bounded context yet — `simulations` is next per the roadmap in `docs/PRD.md` §12 and the module-by-module build rule below (it will need the Reality Engine service from `docs/REALITY_ENGINE.md` to actually produce results, so it's the point where backend work and the — not-yet-started — Reality Engine converge).
...
## Architecture conventions (established by `identity`/`goals`/`decisions` — follow for every new module)
