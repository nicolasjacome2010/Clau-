...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth (Pantallas 1-3 of §2) are implemented** (see `app/`); Pantallas 4-15 (Home, Clarificación, Simulación en vivo, Resultados, Memoria, Suscripción, Ajustes) are still design-only.

Only **Core API modules 1-6 ("identity", "goals", "decisions", "simulations", "memory", "billing")**, **Reality Engine Agents 0-12 (all 13)**, and the **Flutter client's foundation + first feature vertical** (`app/`: design system, routing, networking scaffold, Splash → Onboarding → Auth) have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
app/             Flutter client — SEPARATE Dart/Flutter project, not part of backend/ or reality_engine/
  ...
  lib/
    ...
    features/home/                         # HomePlaceholderScreen only — Pantalla 4 real is a future module
