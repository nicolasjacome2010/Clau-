...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth/Home/Clarificación/Resultados+Comparación+Síntesis+Cierre de ciclo/Mis Decisiones/Memoria/Perfil de Objetivos/Suscripción (Pantallas 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12, 13, 14 of §2) are implemented** (see `app/`); only Pantallas 6 and 15 (Simulación en vivo, Ajustes) are still design-only.
