...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth/Home/Clarificación/Resultados+Síntesis/Mis Decisiones/Memoria/Perfil de Objetivos (Pantallas 1, 2, 3, 4, 5, 7, 9, 10, 12, 13 of §2) are implemented** (see `app/`); Pantallas 6, 8, 11, 14-15 (Simulación en vivo, Comparación, cierre de ciclo, Suscripción, Ajustes) are still design-only.

Only **Core API modules 1-6 ("identity", "goals", "decisions", "simulations", "memory", "billing")**, **Reality Engine Agents 0-12 (all 13)**, and the **Flutter client's foundation + seven feature verticals** (`app/`: design system, routing, networking scaffold, Splash → Onboarding → Auth → Home → Clarificación → Resultados/Síntesis, plus Mis Decisiones / Memoria / Perfil de Objetivos in the nav shell) have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
app/             Flutter client — SEPARATE Dart/Flutter project, not part of backend/ or reality_engine/
  ...
  lib/
    ...
    features/simulations/                  # Pantallas 7 + 9, REAL: ApiSimulationsRepository calls
                                           # GET/POST /v1/decisions/{id}/simulations (90s timeout on
                                           # the POST — the pipeline blocks 15-30s). Opened by tapping
