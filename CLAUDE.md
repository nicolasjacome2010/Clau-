...
- **A setting that can't do anything says so, with the reason.** Ajustes wires appearance for real (persisted per device — a phone in dark at night and a desktop in light by day are the same user, so it is deliberately *not* a server-side profile field) and lists privacy/notifications/language as pending with the missing piece named. A switch that toggles nothing is worse than an absence, and the same rule already governs Memoria's GDPR buttons and Home's mic.
