...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth/Home/Mis Decisiones/Memoria (Pantallas 1, 2, 3, 4, 10, 12 of §2) are implemented** (see `app/`); Pantallas 5-9, 11, 13-15 (Clarificación, Simulación en vivo, Resultados, Perfil de Objetivos, Suscripción, Ajustes) are still design-only.

Only **Core API modules 1-6 ("identity", "goals", "decisions", "simulations", "memory", "billing")**, **Reality Engine Agents 0-12 (all 13)**, and the **Flutter client's foundation + four feature verticals** (`app/`: design system, routing, networking scaffold, Splash → Onboarding → Auth → Home → Mis Decisiones → Memoria) have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
app/             Flutter client — SEPARATE Dart/Flutter project, not part of backend/ or reality_engine/
  ...
  lib/
    ...
    features/decisions/                    # domain shared by Home and Mis Decisiones (not owned by
                                           # either): ApiDecisionsRepository calls GET /v1/decisions for
                                           # real over the shared Dio; DecisionsController fetches ALL
                                           # decisions once, deduped by Riverpod across both screens
    features/home/                         # Pantalla 4, REAL: nav shell (bottom nav mobile /
                                           # NavigationRail tablet+) has 1 of its 4 destinations as a
                                           # placeholder (Perfil); Home + Mis Decisiones are real
    ...
    features/shared/                       # FeaturePlaceholderContent — reused by nav-shell destinations
                                           # whose real screen is a future module
