...
4. `docs/DATABASE.md` — full normalized schema. Only the `identity` slice (`users`, `user_profiles`) is implemented so far; the rest is design-only.
...
Only **Phase 6, module 1 ("identity")** of the backend has been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
backend/         Core API service (Python/FastAPI, Clean Architecture monolith — see docs/ARCHITECTURE.md §4)
  src/core_api/
    ...
    identity/domain|application|infrastructure|api # the one bounded context implemented so far
...
There is no Flutter client, Reality Engine service, Billing service, or any bounded context beyond `identity` yet — those are the next modules per the roadmap in `docs/PRD.md` §12 and the module-by-module build rule below.
...
## Architecture conventions (established by the `identity` module — follow for every new module)
