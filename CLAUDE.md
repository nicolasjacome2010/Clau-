...
5. `docs/UX_DESIGN.md` — screen-by-screen design system (color, type, motion, wireframes) for the Flutter client. **Design system + Splash/Onboarding/Auth/Home (Pantallas 1-4 of §2) are implemented** (see `app/`); Pantallas 5-15 (Clarificación, Simulación en vivo, Resultados, Memoria, Suscripción, Ajustes) are still design-only.

Only **Core API modules 1-6 ("identity", "goals", "decisions", "simulations", "memory", "billing")**, **Reality Engine Agents 0-12 (all 13)**, and the **Flutter client's foundation + first two feature verticals** (`app/`: design system, routing, networking scaffold, Splash → Onboarding → Auth → Home) have been built. Everything else in those docs is design, not yet code — don't assume a feature exists just because it's documented.
...
app/             Flutter client — SEPARATE Dart/Flutter project, not part of backend/ or reality_engine/
  ...
  lib/
    ...
    features/home/                         # Pantalla 4, REAL: ApiDecisionsRepository calls GET /v1/decisions
                                           # for real over the shared Dio; nav shell (bottom nav mobile /
                                           # NavigationRail tablet+) has 3 of its 4 destinations as placeholders
