...
src/core_api/
  ...
  simulations/
    ...
    api/                              # /v1/decisions/{id}/simulations, /v1/simulations/{id},
                                        # /v1/decisions/{id}/simulations/stream (NDJSON, relay del
                                        # stream de reality_engine), /v1/decisions/{id}/outcome,
                                        # /v1/outcomes
...
Cuando una simulación se completa y el Reality Engine incluyó un resumen de memoria (Agente 11), `RunSimulationUseCase` lo persiste automáticamente en `memory` — sin bloquear la simulación si el Reality Engine no lo produjo.

`POST /v1/decisions/{id}/simulations/stream` es la misma operación que `POST /v1/decisions/{id}/simulations`, reportando el progreso a medida que ocurre (docs/UX_DESIGN.md Pantalla 6): relee el NDJSON de `reality_engine`'s `/v1/simulate/stream` línea por línea (`HttpRealityEngineClient.simulate_stream`) y lo relee hacia el cliente como su propio NDJSON, nunca reexportando la forma de Reality Engine — cada línea de etapa se traduce a `SimulationStageEvent` (el tipo propio de este contexto) antes de salir, mismo principio de puerto+DTOs propios que el resto de esta integración. `RunSimulationUseCase` y `RunSimulationStreamUseCase` comparten toda la lógica de qué es una simulación completada a través de una base común (`_SimulationRunner._persist_outcome`) — solo difiere cómo se entera el caller del progreso, nunca la definición de "completada". La comprobación de propiedad de la decisión ocurre *antes* de que arranque el stream: como `StreamingResponse` envía su 200 y sus headers en cuanto se pide el primer chunk, un `DecisionNotFoundError` levantado dentro del generador ya sería demasiado tarde para convertirse en un 404 real, así que el router avanza el generador del caso de uso un paso (`await anext(...)`) todavía dentro del flujo de request/response ordinario, y solo envuelve el resto en `StreamingResponse`. Un fallo que ocurre *después* de que el 200 ya salió (por ejemplo, Reality Engine cae a mitad de la conexión) no puede volverse un código de error HTTP — se informa en banda, como una línea `{"type": "error", ...}`, porque un cuerpo que termina antes de tiempo sin esa línea sería indistinguible de una conexión caída.

`POST /v1/decisions/{id}/outcome` cierra el ciclo (docs/PRD.md CU8): reenvía los escenarios de la última simulación completada al Reality Engine (`/v1/calibrate`, Agente 12) junto con lo que el usuario reporta que pasó realmente, y persiste tanto el `DecisionOutcome` como el efecto en `UserBiasProfile`. **Un ciclo se cierra una sola vez**: reportar de nuevo devuelve 409 y la comprobación corre *antes* de llamar al Reality Engine, así que un duplicado no gasta una llamada al modelo ni pliega dos veces la misma observación en el perfil de sesgos del usuario; `decision_outcomes.decision_id` es además único en la base (migración 0008), que es lo único que sostiene el invariante si dos reportes concurrentes pasan la guardia a la vez. `GET /v1/outcomes` devuelve todos los cierres de ciclo del caller en una sola llamada — es una colección y no `GET /decisions/{id}/outcome` porque la pregunta real del cliente es "¿cuáles de mis decisiones siguen abiertas?", y responderla de a una sería un N+1 contra una pantalla de lista (docs/UX_DESIGN.md Pantalla 10).
